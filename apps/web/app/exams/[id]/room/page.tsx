...
import { saveAnswer, getDraft, clearDraft } from "@/lib/storage/exam-store";
...
export default function ExamRoomPage({ params }: ExamRoomProps) {
  ...
  useEffect(() => {
    ...
    async function loadExam() {
      try {
        ...
        const draft = await getDraft(examId);
        if (draft && draft.answers && isMounted) {
  ...
  }, [timeRemainingSeconds, handleFinalSubmit]);

  // 4. Anti-Cheat Protocol: visibilitychange & window blur
  useEffect(() => {
  ...
  }, [examId, infractionCount, handleFinalSubmit]);

  // 5. Fullscreen Request
  ...
  const handleSelectOption = async (questionId: string, optionId: string) => {
    ...
    await saveAnswer(examId, questionId, optionId, serverOffset);
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 text-zinc-600">
        ...
        <span className="ml-3 text-sm font-medium">প্রশ্নপত্র লোড হচ্ছে...</span>
      </div>
    );
  }
